...
<div class="container-fluid">
    <header class="d-flex justify-content-between align-items-center py-3 bg-dark text-white">
        ...
        <nav>
            <ul class="nav">
                ...
                <li class="nav-item">
                    <a class="nav-link text-white" href="/administrator/logout"
                       style="color: #f44336; font-weight: bold; text-transform: uppercase;">
                        退出
                    </a>
                </li>
    ...
    <main class="container py-5">
        <h2 class="text-center mb-4">轮播图</h2>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addUserModalLabel">添加轮播图</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="addUserForm" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="addUsername" class="form-label">名称</label>
                        <input type="text" name="username" class="form-control" id="addUsername" required>
                    </div>
                    <div class="mb-3">
                        <label for="addPassword" class="form-label">图片</label>
                        <input type="file" name="file" class="form-control" id="addPassword" accept="image/*" required>
                    </div>
...
<div class="modal fade" id="editUserModal" tabindex="-1" aria-labelledby="editUserModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editUserModalLabel">修改轮播图</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editUserForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label for="editUsername" class="form-label">名称</label>
                        <input type="text" name="username" class="form-control" id="editUsername" required>
                    </div>
                    <div class="mb-3">
                        <label for="editPassword" class="form-label">图片</label>
                        <input type="file" name="file" class="form-control" id="editPassword" required>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#addUserForm').submit(function (event) {
            ...
            $.ajax({
                url: '/lunbotu/add',  // 添加用户的Servlet路径
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
        ...
        $('#editUserForm').submit(function (event) {
            ...
            var formData = new FormData(this);
            $.ajax({
                url: '/lunbotu/update',  // 更新用户的Servlet路径
                type: 'POST',
        ...
        $('#confirmDeleteBtn').click(function () {
            if (selectedUserId != null) {
                $.ajax({
                    url: '/lunbotu/delete',  // 删除用户的Servlet路径
                    type: 'POST',
            ...
            success: function (data) {
                ...
                if (data && data.users) {
                    ...
                    if (data.users.length === 0) {
                        $('#userList').append('<tr><td colspan="4" class="text-center">暂无轮播图</td></tr>');
                    }
                    ...
                    data.users.forEach(function (user) {
                        ...
                        row += '<td>' +
                            '<img src="' + user.password + '"  style="width:50px;height:50px;"/>' + '</td>';  // 显示密码
                        row += '<td>';
