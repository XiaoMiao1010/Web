...
<script type="text/javascript">
  ...
  // 获取从后台返回的商品数据
  fetch('/getPD')  // 获取第一个图表数据的Servlet路径
          .then(response => response.json())
          .then(data => {
            // 从后台返回的数据格式：[{categoryName: '电子产品', productCount: 120}, {...}]
            console.log(data)
            // 准备第一个图表数据
            const categories = data.names;  // 商品种类
            const productCounts = data.counts;  // 商品数量

            // 创建第一个图表实例：商品种类分布图
            var categoryChart = echarts.init(document.getElementById('p3'));
